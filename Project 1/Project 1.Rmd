...
visa_distribution <- data %>% 
  group_by(Country, Fiscal.Year, Visa.Category) %>% 
  summarise(Total.Visas = sum(Quantity.of.U.S..Visas.Granted), .groups = 'drop')

# Identify top 5 countries by total visa issuances
top_countries <- visa_distribution %>% 
  group_by(Country) %>% 
  summarise(Total.Visas = sum(Total.Visas)) %>% 
  top_n(5, Total.Visas) %>% 
  pull(Country)
...
p <- ggplot(filtered_data, aes(x = Fiscal.Year, y = Total.Visas, fill = Visa.Category)) +
  ...
  labs(
    title = 'Stacked Bar Plot of Visa Issuance for Top 5 Countries',
    x = 'Fiscal Year',
  ...
  theme(
    ...
    aspect.ratio = 0.6  # Make the plot wider
  )
