---
title: "Analyzing U.S. Visa Issuance Trends: The Impact of Policy, Economic Factors, and Demographic Shifts"
...
The graph highlights distinct trends in visa issuance across presidential administrations, emphasizing how policy priorities have shaped the relative allocation of visa categories. Under President George W. Bush, skilled labor and student/exchange visitor visas experienced modest growth, despite the restrictive environment created by post-9/11 security measures. These increases suggest a measured approach that balanced national security with economic and educational priorities.

The Obama administration saw a marked expansion in visa issuance, particularly for student and exchange visitor visas, reflecting policies that prioritized international collaboration and educational exchange. Skilled labor visas also grew steadily during this time, aligning with efforts to support economic recovery and incentivize employment-based immigration.

Meanwhile, the Trump administration marked a stark departure from these trends, with declines across all major visa categories even prior to the pandemic-induced drop. This period marked the first time in decades where unskilled labor visas surpassed skilled labor visas in issuance, reflecting the administration's preference for labor migration tied to seasonal and temporary work programs over pathways that might facilitate long-term immigration. Student and exchange visitor visas experienced the sharpest decline during this period, partially driven by more stringent eligibility requirements and an overarching restrictive stance on immigration.

Under the Biden administration, visa issuance shows signs of recovery, particularly in the student and exchange visitor category, where numbers have rebounded rapidly. Skilled labor visas are also recovering, though unskilled labor visas continue to maintain a notable share, suggesting an ongoing emphasis on labor migration to address workforce needs. While this rebound is partially attributable to the relaxation of COVID-19 restrictions, it may also reflects a policy shift toward rebuilding immigration channels after a period of contraction.
...
visa_colors <- c(
  "Skilled Labor Visas" = "#F84947",
  "Unskilled Labor Visas" = "#F1C40F",
  "Student and Exchange Visitor Visas" = "#5794D4"
)
...
p <- ggplot(population_growth_data, aes(x = `Fiscal Year`, y = Population, color = Country)) +
  ...
  # Consistent Styling with Visa Issuance Chart
  theme_minimal() +
  theme(
    # Title and subtitle
    plot.title = element_text(hjust = 0.5, family = "bodoni_moda", size = 16, face = "bold"),
...
p <- ggplot(merged_data, aes(x = Population / 1e6, y = Total.Visas / 1e6)) +
  ...
  geom_point(aes(color = as.factor(Fiscal.Year)), size = 3) +
  ...
  geom_line(aes(x = Population / 1e6, y = log1p(RateOfNaturalIncrease)), color = "#A93226", size = 1, linetype = "dashed") +
  
  # Annotate Rate of Natural Increase Label
  annotate("text", 
           x = max(merged_data$Population / 1e6) - 50, 
           y = log1p(max(merged_data$RateOfNaturalIncrease)), 
           label = "Rate of Natural Increase", 
           color = "#A93226", 
           hjust = 1, vjust = -0.5, 
           size = 4, family = "montserrat", fontface = "italic") +
  ...
  theme(
    ...
    axis.text = element_text(family = "montserrat", size = 10),
    axis.text.x = element_text(angle = 45, hjust = 1),
    legend.title = element_text(family = "montserrat", size = 12),
    legend.text = element_text(family = "montserrat", size = 10),
    legend.position = 'right',
    plot.margin = margin(20, 20, 40, 20) # Adjust margins to prevent cutoff
  ) +
  ...
  # Enhanced contrasting color gradient
  scale_color_manual(values = colorRampPalette(c("#F84947", "#F1C40F", "#092E86"))(length(unique(merged_data$Fiscal.Year))))

# Display the plot
print(p)

# Save the plot
ggsave("us_population_vs_visas_rni_contrasting_log_1997_2022.png", plot = p, width = 12, height = 8, dpi = 300)
```
...
- Migration Policy Institute (n.d.). *Migration Policy Institute Website*. Available at: [https://www.migrationpolicy.org/](https://www.migrationpolicy.org/).

- Preston, S. and Elo, I. (1995). "Demography: Analysis of Mortality and Fertility Rates." *Journal of Monetary Economics*, 36(1), pp. 135–157. Available at: [https://doi.org/10.1016/S0304-3932(02)00206-4](https://doi.org/10.1016/S0304-3932(02)00206-4).

- U.S. Department of Labor (n.d.). *H-1B Visa Program Overview*. Available at: [https://www.dol.gov/agencies/whd/immigration/h1b](https://www.dol.gov/agencies/whd/immigration/h1b).

- Center for Comparative Immigration Studies (n.d.). *Understanding Visa Issuance Trends*. Available at: [https://ccis.ucsd.edu/_files/wp11.pdf](https://ccis.ucsd.edu/_files/wp11.pdf).

- Migration Policy Institute (n.d.). *H-1B Temporary Skilled Worker Program*. Available at: [https://www.migrationpolicy.org/article/h-1b-temporary-skilled-worker-program](https://www.migrationpolicy.org/article/h-1b-temporary-skilled-worker-program).

- Rice, I. (n.d.). *Timeline of Student Visa Policy in the United States*. Available at: [https://timelines.issarice.com/wiki/Timeline_of_student_visa_policy_in_the_United_States](https://timelines.issarice.com/wiki/Timeline_of_student_visa_policy_in_the_United_States)
