...
total_article_count <- nrow(doc_data)
print(paste("Total number of articles:", total_article_count))

# Count occurrences of "not" in all documents
total_not_count <- sum(str_count(doc_data$Text, regex("\\bnot\\b", ignore_case = TRUE)))
print(paste("Total occurrences of 'not':", total_not_count))

# Count articles per year
year_counts <- table(doc_data$Year)

# Plot article count by year using base plotting
 plot(year_counts, type = "o", main = "Articles per Year 1997-2022", xlab = "Year", ylab = "Number of Articles",
       col = "blue", pch = 19, lty = 1, lwd = 2)
  text(x = as.numeric(names(year_counts)), y = year_counts, labels = year_counts, pos = 2, cex = 0.8, col = "black")
