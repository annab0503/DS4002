...
3. **How has the specific language used in media coverage to discuss H-1B visas shifted over time?** By investigating the most frequently occurring word pairs (key bigrams) in these articles, this question uncovers dominant themes and recurring narratives, offering deeper insights into the framing of H-1B visa discussions.  
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
# Step 7: Multiply Sentiment by the Number of Articles per Year
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
# Step 8: Add dashed lines for presidential administrations
# Define key transition years for administrations
admin_years <- c(2001, 2008, 2015, 2020)  # Adjust these years based on administrations
admin_labels <- c("Bush", "Obama", "Trump", "Biden")
```

```{r fig.width=10, fig.height=6, echo=FALSE, warning=FALSE, message=FALSE}
# Step 9: Visualize Adjusted Sentiment Over Time with Presidential Administration Lines
ggplot(sentiment_with_articles, aes(x = as.numeric(Year), y = AdjustedSentiment)) + 
  geom_line(color = "#5794D4") + 
  geom_point(color = "#5794D4") + 
  ...
  theme(
    # Title customization
    plot.title = element_text(hjust = 0.5, family = "bodoni_moda", size = 16, margin = margin(b = 15), face = "bold"),
    plot.subtitle = element_text(hjust = 0.5, family = "montserrat", size = 12, margin = margin(b = 10)),  # Customize subtitle
    # Axis title and text customization
    axis.title = element_text(family = "montserrat", size = 14),
...
## **How has the specific language used in media coverage to discuss H-1B visas shifted over time?**
