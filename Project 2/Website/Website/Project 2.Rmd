...
```{r message=FALSE, include=FALSE}
# Function to get top bigrams for each year
get_top_bigrams_by_year <- function(df) {
  df %>%
    unnest_tokens(bigram, Concatenated_Articles, token = "ngrams", n = 2) %>%  # Extract bigrams
    separate(bigram, c("word1", "word2"), sep = " ") %>%  # Separate the bigram into words
    filter(!word1 %in% stop_words$word & !word2 %in% stop_words$word) %>%  # Remove stop words
    filter(word1 != "" & word2 != "") %>%  # Remove blank spaces
    unite(bigram, word1, word2, sep = " ") %>%  # Recombine the words into bigrams
    count(Year, bigram, sort = TRUE)  # Count bigrams by year
}

# Apply the function to grouped_data to get bigrams per year
top_bigrams_by_year <- grouped_data %>%
  group_by(Year) %>%
  do(get_top_bigrams_by_year(.))

# View the top bigrams by year
print(top_bigrams_by_year)

# Optionally, get the top 5 bigrams for each year
top_bigrams_by_year_top5 <- top_bigrams_by_year %>%
  group_by(Year) %>%
  top_n(3, n)  # Get top 5 bigrams for each year
```

```{r fig.width=10, fig.height=8, echo=FALSE, warning=FALSE, message=FALSE}
# Plotting the top bigrams by year
ggplot(top_bigrams_by_year_top5, aes(x = reorder(bigram, n), y = n, fill = as.factor(Year))) + 
  ...
  theme(
    ...
    # Axis title and text customization
    axis.title = element_text(family = "montserrat", size = 10),
    axis.text = element_text(family = "montserrat", size = 8),
    
    # Text customization for general appearance
    text = element_text(size = 12, family = "montserrat"),
